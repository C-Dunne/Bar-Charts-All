class StackedChart {
    constructor(obj) {
        ...
        // to get totals for scaling
        let barTotals = {};
        for (let i = 0; i < this.data.length; i++) {
            let row = this.data[i];
            let bar = row[this.xValue];

            // check if xValue already in barTotals obj
            if (!barTotals[bar]) {
                // if not start at 0
                barTotals[bar] = 0;
            }
            ...
            for (let y of this.yValues) {
                // add each yvalue from our xvalue to total
                barTotals[bar] += row[y];
            }
        }
        let totalValue = Object.values(barTotals);
        this.maxValue = max(totalValue);
    ...
    render() {
        ...
        for (let i = 0; i <= this.numTicks; i++) {
            ...
            line(0, 0, -5, 0);
            let cascadeGap = round(this.maxValue) / this.numTicks;
       ...
       //legend
        ...
        for (let i = 0; i < this.data.length; i++) {
            ...
            push();
            for (let y = 0; y < this.yValues.length; y++) {
                // loop for colours + drawing bars. for y less than the length of the stacked barchart values
                fill(StackColours[y % StackColours.length]);
                // fill based on an array of set colours in sketch, based on amount of values in the yAxis
                let barHeight = -this.data[i][this.yValues[y]] * this.scale;
                // barheight = access this.data[5][1] * this.scale
                // this.data[i] = numbers in xvalue ie. 55-64 years
                // this.data[i][this.values[y]] = numbers in specific yvalue ie. 11 for male in 65+ years
                rect(gap, 0, this.barWidth, barHeight);
